...
export const validateProduct = [
  ...
  body("product_name")
    .trim()
    .notEmpty()
    .withMessage("Produktnamn måste anges")
    .isLength({ min: 3, max: 100 })
    .withMessage("Produktnamnet måste vara mellan 3 och 100 tecken"),

  // Validate description
  body("description")
    .optional()
    .isString()
    .withMessage("Beskrivningen måste vara en text")
    .isLength({ max: 500 })
  ...
  body("variants").custom((variants) => {
    ...
    variants.forEach((size) => {
      ...
      if (
        !Number.isInteger(parseInt(size.price)) ||
        parseInt(size.price) <= 0
      ) {
        throw new Error("price måste vara ett positivt nummer");
      }
      if (
        !Number.isInteger(parseInt(size.stock_quantity)) ||
        parseInt(size.stock_quantity) < 0
      ) {
  ...
  body("usage_products")
    .optional()
    .isString()
    .withMessage("Användningsområde måste vara en text")
    .isLength({ max: 500 })
    .withMessage("Användningsområde får inte vara längre än 500 tecken"),

  // Validate ingredients
  body("ingredients")
    .optional()
    .isString()
    .withMessage("Ingredienser måste vara en text")
    .matches(/^([^,]+(, )?)+$/)
    .withMessage("Ingredienser ska vara kommaseparerade strängar"),

  // Validate category_id
  body("category_id")
    .notEmpty()
    .withMessage("category_id måste anges")
    .isInt({ gt: 0 })
    .withMessage("category_id måste vara ett positivt heltal"),

  // Validate featured as a boolean
  body("featured")
    .optional()
    .isBoolean()
    .withMessage("featured måste vara en boolean"),

  // Validate properties as an array of objects with specific structure
  body("properties").custom((properties) => {
    ...
    properties.forEach((property) => {
      ...
      if (
        !Number.isInteger(parseInt(property.property_id)) ||
        parseInt(property.property_id) <= 0
      ) {
        throw new Error(
          "Varje egenskap måste ha ett positivt heltal för property_id"
        );
      }
    });
    return true;
  ...
  body("brand_id")
    .notEmpty()
    .withMessage("brand_id måste anges")
    .isInt({ gt: 0 })
    .withMessage("brand_id måste vara ett positivt heltal"),
...
export const createProduct = [
  ...
  async (req, res) => {
    ...
    const primaryImagePath = req.files.primaryImage
      ? req.files.primaryImage[0].filename
      : null;
    const secondaryImagePath = req.files.secondaryImage
      ? req.files.secondaryImage[0].filename
      : null;
    const thirdImagePath = req.files.thirdImage
      ? req.files.thirdImage[0].filename
      : null;
    ...
    try {
      ...
      const fetchedCategories = await getCategoriesById(
      ...
      const productWithInsertId = {
        ...
        brand: {
          ...
          brand_name: fetchedBrands.brand_name,
        },
        category: {
